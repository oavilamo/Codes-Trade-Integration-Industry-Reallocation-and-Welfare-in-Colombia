...
@#for co in Country
   ...
   Lp_T_@{co} GO_T_@{co} I_T_@{co} I_T_@{co}_real lambda_@{co}
@#endfor
...
@#for sec in Sectors
    %phi_k_SOE@{sec} = 0.01/25;    
    %phi_k_ROW@{sec} = 0.01/25;
...
W_@{co}*f0_@{co}@{sec} = n_@{co}*Q_@{co}*(EV_1_@{co}@{sec}(+1)-EV_inf_@{co}@{sec}(+1))
@#for co1 in Country
    @#if co1!=co
...
@#for co in Country
[name='Walras']
    WL_@{co} = P_C_@{co}*C_@{co} - TauT_@{co} - PI_T_@{co} - W_@{co}*L_@{co}  
@#for sec in Sectors
...
%Bond_SOE = [linspace(0,0.4,2) linspace(0.6,0,28)];
%Bond_ROW = [linspace(0,-0.4,2) linspace(-0.6,0,28)];
...
var eps_SOE5ROW; 
periods 1:2;
values (tariff_SOE5ROW);
var eps_SOE6ROW; 
periods 1:2;
values (tariff_SOE6ROW);
end;
