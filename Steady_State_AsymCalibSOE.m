function [SS0, P_C, Q, C, W, NT, P, K, P_K, R_K, X_C, I, D_K, X_K, X, D, MC, PI_0, a_p, a_0, a_1, ...
    ...
    GO_EX, GO_NoEX, EX_0, EX_1, P_M, mm, X_M, M, D_M, MC_CD, CD, Lp_T, EX_S, Exporter_Prem, TB] = Steady_State_AsymCalibSOE(x)

global beta delta theta eta n sigma omega_c omega_k omega alpha theta_s fe fp f0 f1 xi_0 xi_1 tau S CO L  ...
     kappa sigma_m lambda Exporter_SOE New_exporter_SOE EX_new_SOE Exporter_Prem_SOE TO_SOE A
index = 1;
...
f1(1,i) = x(index); index = index+1;
xi_0(1,i,2) = x(index); index = index+1;
xi_1(1,i,2) = x(index); index = index+1;
end
...
for i = 1:S
    ...
    SS0(index) = New_exp(1,i) - New_exporter_SOE(1,i); index = index+1;
    SS0(index) = EX_new(1,i) - EX_new_SOE(1,i); index = index+1;
