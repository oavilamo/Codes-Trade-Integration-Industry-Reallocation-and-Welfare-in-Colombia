...
function [SS0, P_C, Q, C, W, NT, P, K, P_K, R_K, X_C, I, D_K, X_K, X, D, MC, PI_0, a_p, a_0, a_1, ...
    ...
    EX_1, EX_0, GO, GO_EX, GO_NoEX, EX_GO, TO, Exporter, New_exp, Exporter_Prem, EX_new, mu, D_M] = Steady_State_AsymSectorCalib(x)
...
for i = 1:S
P(1,i) = x(index); index = index+1;
K(1,i) = x(index); index = index+1;
a_0(1,i) = x(index); index = index+1;
M(1,i) = x(index); index = index+1;
end
...
f0(2,i) = f0(1,i);
f1(2,i) = f1(1,i);
xi_0(2,i,1) = xi_0(1,i,2); 
xi_1(2,i,1) = xi_1(1,i,2); 
end
